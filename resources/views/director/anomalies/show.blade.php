<x-layout.sidebar title="Record Detail">
    ...
    <div class="grid grid-cols-1 gap-5 xl:grid-cols-2">
        ...
        <x-hp.card>
            ...
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between gap-4">
                    <dt class="text-hp-slate/50 dark:text-hp-slate/60">Student</dt>
                    <dd class="font-semibold text-hp-slate">{{ $visit->student->name ?? '—' }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-hp-slate/50 dark:text-hp-slate/60">Student number</dt>
                ...
                <div class="flex justify-between gap-4">
                    <dt class="text-hp-slate/50 dark:text-hp-slate/60">College at visit</dt>
                    <dd class="text-hp-slate">{{ $visit->college->name ?? '—' }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                    <dt class="text-hp-slate/50 dark:text-hp-slate/60">Checked in</dt>
        ...
        <x-hp.card>
            ...
            @else
                <dl class="space-y-3 text-sm">
                    <div class="flex justify-between gap-4">
                        <dt class="text-hp-slate/50 dark:text-hp-slate/60">Result</dt>
                        <dd>
                    ...
                    <div class="flex justify-between gap-4">
                        <dt class="text-hp-slate/50 dark:text-hp-slate/60">Encoded</dt>
    ...
    <x-hp.card class="mt-5">
        <div class="mb-4 flex items-center justify-between">
            ...
            @if ($visit->vitalSigns)
                <span class="text-xs text-hp-slate/40 dark:text-hp-slate/55">
        ...
        @if ($visit->vitalSigns === null)
            <p class="py-6 text-center text-sm text-hp-slate/50 dark:text-hp-slate/60">No vitals recorded for this visit.</p>
        @else
            @php $vitals = $visit->vitalSigns; @endphp
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-widest text-hp-slate/40 dark:text-hp-slate/55">Height</p>
                    <p class="mt-1 text-lg font-bold text-hp-slate">{{ $vitals->height_cm }} cm</p>
                </div>
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-widest text-hp-slate/40 dark:text-hp-slate/55">Weight</p>
                    <p class="mt-1 text-lg font-bold text-hp-slate">{{ $vitals->weight_kg }} kg</p>
                </div>
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-widest text-hp-slate/40 dark:text-hp-slate/55">BMI</p>
                ...
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-widest text-hp-slate/40 dark:text-hp-slate/55">Temperature</p>
                ...
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-widest text-hp-slate/40 dark:text-hp-slate/55">Heart Rate</p>
                    <p class="mt-1 text-lg font-bold text-hp-slate">{{ $vitals->heart_rate_bpm }} bpm</p>
                </div>
                <div>
                    <p class="text-[11px] font-semibold uppercase tracking-widest text-hp-slate/40 dark:text-hp-slate/55">Blood Pressure</p>
