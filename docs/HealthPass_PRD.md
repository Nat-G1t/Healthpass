...
| D-35 | ⚠️ **NOT YET IMPLEMENTED — flagged schema change (`users.must_change_password` boolean, default false).** **Staff accounts stay seeded (FR-AUTH-05 unchanged) but ship with real emails and one-time passwords that must be changed at first login.** `StaffSeeder` keeps creating the 1 director + 1 nurse + 12 college admins, with the `@healthpass.test` placeholders and shared `password` replaced by real DHVSU addresses and per-account random passwords; each seeded staff user is flagged `must_change_password = true`, and middleware redirects them to the existing D-20 change-password screen until they clear it. Students are unaffected (they self-register and set their own password). **Rejected alternative:** a public/College-Admin self-registration flow approved by the Director — it contradicts **FR-AUTH-05 (M)** ("no public staff registration path shall exist"), and would add screens, states, notification mail, and tests to provision **14 fixed accounts** that change roughly never. | Going live on the internet with 14 accounts sharing the password `password` and `@healthpass.test` addresses is the single largest exposure D-34 introduces — the same seeder that is correct for a laptop demo is a breach on a public domain. Forcing the change at first login keeps provisioning a one-command seed (no new screens, no approval workflow) while guaranteeing no shipped credential survives contact with production. An approval workflow would be a new feature solving a problem the fixed, tiny staff roster does not have. |
